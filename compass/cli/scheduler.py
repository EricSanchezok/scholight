"""CLI: ``compass scheduler`` — daily arxiv pipeline daemons.

Usage::

    compass scheduler paper-sync     # one-shot sync (same as compass service sync)
    compass scheduler pdf-daemon     # long-running PDF download daemon
    compass scheduler md-daemon      # long-running markdown parse daemon
    compass scheduler chunk-daemon   # long-running chunk ingest daemon
    compass scheduler status         # pipeline progress report
"""
...
def paper_sync_cmd() -> None:
    ...
    from compass.scheduler.arxiv_paper_sync import run_sync
...
def pdf_daemon_cmd() -> None:
    ...
    from compass.scheduler.pdf_download import PdfDownloadDaemon
...
def md_daemon_cmd() -> None:
    ...
    from compass.scheduler.md_parse import MdParseDaemon
...
def chunk_daemon_cmd() -> None:
    ...
    from compass.scheduler.chunk_ingest import ChunkIngestDaemon
