"""CLI: ``compass store init``, ``compass store status``, ``compass store drop``,
``compass store backup``, ``compass store restore``, ``compass store health``."""
...
from compass.store.client import close, connect, is_connected
from compass.store.export import export_collection_to_path, restore_collection_from_path
from compass.store.schema import create_collections, create_indexes, drop_collections
...
def health(
    ...
    \b
    Examples:
        compass store health                    # quick check
        compass store health --deep             # full analysis
        compass store health -d indexes -d vectors  # specific layers only
        compass store health --fix              # auto-fix recoverable issues
        compass store health -o json            # machine-readable output
    """
    from compass.store.health import run_health_check
...
def restore(input_dir: str, batch_size: int) -> None:
    ...
    Example: ``compass store restore backups/logical/20260530/arxiv_papers``
    """
