"""Unit tests for compass.store.ingest."""
...
class TestUpdateArxivPaper:
    def test_update_sets_has_pdf_true(self) -> None:
        ...
        with patch("compass.store.ingest.get_client", return_value=mock_client):
...
class TestDeleteArxivPaper:
    def test_delete_returns_true_when_paper_exists(self) -> None:
        ...
        with patch("compass.store.ingest._delete_chunks_locked", MagicMock()):
            with patch("compass.store.ingest.get_client", return_value=mock_client):
    ...
    def test_delete_deletes_chunks_before_paper(self) -> None:
        ...
        with patch("compass.store.ingest._delete_chunks_locked", mock_delete_chunks):
            with patch("compass.store.ingest.get_client", return_value=mock_client):
...
class TestUpsertArxivPapers:
    def test_upsert_raises_with_committed_count(self) -> None:
        ...
        with patch("compass.store.ingest.batched", side_effect=_one_per_batch):
            with patch("compass.store.ingest.get_client", return_value=mock_client):
