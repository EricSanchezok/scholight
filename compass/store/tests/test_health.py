"""Unit tests for compass.store.health — Zilliz Cloud health checker (4 resource flags)."""
...
from compass.store.health import (
    CheckResult,
    HealthChecker,
    HealthReport,
    HealthStatus,
...
class TestCheckConnection:
    def test_pass_connected(self) -> None:
        mock = _mock_client()
        with patch("compass.store.health.get_client", return_value=mock):
...
class TestHealthCheckerRun:
    ...
    def test_run_with_dims_filter(self) -> None:
        ...
        layer_names = [l.layer for l in report.layers]
    ...
    def test_run_handles_layer_exception(self) -> None:
        ...
        assert len(report.layers) == 8
        assert any("exception" in c.name for l in report.layers for c in l.checks)
