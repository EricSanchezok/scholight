...
from compass.store.client import QUERY_CONSISTENCY, SEARCH_CONSISTENCY, get_client
...
def hybrid_search_arxiv_papers(
    ...
    *,
    ...
    ``COMPASS_SEARCH_HYBRID_DENSE_WEIGHT`` and ``COMPASS_SEARCH_HYBRID_BM25_WEIGHT``.
    """
    client = get_client()
    fields = output_fields if output_fields is not None else list(PAPER_SEARCH_FIELDS)
    filter_expr = _build_filter(
