...
Usage:
    from compass.store.health import HealthChecker, HealthReport
...
from compass.store.client import escape_sql, get_client
from compass.store.fields import PAPER_SEARCH_FIELDS, PAPER_VECTOR_FIELDS
...
class HealthReport:
    ...
    def print(self) -> str:
        ...
        lines.append(
            f"\n{'=' * 60}"
            f"\n  Academic Compass — Health Report{' (DEEP)' if self.deep else ' (QUICK)'}"
...
class HealthChecker:
    """Run health checks against the Academic Compass Zilliz Cloud database.
