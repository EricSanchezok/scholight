...
- `survey`: run Survey draft/worker processes against local PostgreSQL and MinIO. Survey
  ...
  PDF report rendering also uses the API `matplotlib` extra; formula images are generated locally
  and no report resource is fetched from the network.
...
The archive integration and hermetic Survey E2E stacks pull their digest-pinned MinIO image from the
