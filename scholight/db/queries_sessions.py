...
async def query_sessions(user_id: int, *, client_id: str) -> list[SessionRecord]:
    ...
    try:
        rows = await get_pool().fetch(
            ...
            "FROM auth.refresh_tokens WHERE user_id = $1 AND client_id = $2 "
            "GROUP BY family_id "
...
async def register_session_metadata(
    *, user_id: int, session_id: int, client_id: str, user_agent: str | None
) -> None:
    ...
    try:
        await get_pool().execute(
            "UPDATE auth.refresh_tokens SET user_agent = $4, last_seen_at = now() "
...
async def touch_session(user_id: int, session_id: int, *, client_id: str) -> bool:
...
async def revoke_session(*, user_id: int, session_id: int, client_id: str) -> bool:
    try:
...
async def revoke_other_sessions(*, user_id: int, current_session_id: int, client_id: str) -> int:
    try:
